...
do {
    ...
    function output() {
        if (attemptsAllowed === Infinity) {
            console.log(`\nYour guess: ${guess}`.magenta);
        } else {
            console.log(`\nYour guess: ${guess} | Attempts remaining: ${attemptsAllowed - madeAttempts}`.green);
        }
    
        if (bull === 0 && cow === 0) {
            guess = prompt(`0 bulls and 0 cows! ${displayRandomMessage()} What will your next guess be? `)
        } else {
            guess = prompt(`Bulls: ${bull} | Cows: ${cow}. What will your next guess be? `)
        }
        validationAndResult();
    }
...
function calculation() {
    ...
    for (let i = 0; i < number.length; i++) {
        if (number[i] !== guess[i] && number.includes(guess[i])) {
            cow++;
        } else if (number[i] === guess[i]) {
            bull++;
        }
    }
}
